...
import dklib.histnd
import dklib.arrhenius_loading_11 as arrhenius_loading

import dklib.mf_epm_cyc as mf_epm
import dklib.mf_avalanche as mf_avalanche

import sim_cyc_params_01 as sim_params
...
# def apply_kicks(mf, failingSites, mf_wells=None):
#     global dsig_temp
...
if newSimulation:
    ...
    #assigning the wells to the initial weibull distribution.
    mf.sigma_th_wells = [ {0:threshold} for threshold in mf.sigma_th ] 
...
while True:
    ...
    t1 = time.time()
    mf.apply_kicks(failing_sites, kickSource, rng, renewal_k, cumulative_eigenstrain=cumulative_eigenstrain, constant_stress=False)
    t2 = time.time()
    ...
    while True:
        ...
        if is_root():
            if not use_arrhenius:
                assert False, "we should always be using arrhenius in this version of the code"
            else:
