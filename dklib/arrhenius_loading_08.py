...
# @profile
def load_to_fail_inter_avalanche(mf,rng,tnumerical=1e-6*25,tmax=1e99,debug_level = -1):
...
# @profile
def load_to_fail_analytical(mf,uplus,uminus,tFailFunc,tmax=1e99,debug_level = -1):
    ...
    if(debug_level > 2):
        tot = np.sum(Tplus<0)+np.sum(Tminus<0)
        print('checks for negative time: ',np.sum(Tplus<0),np.sum(Tminus<0))
        if(tot > 0):
            import pdb; pdb.set_trace()
            badind = np.argmin(Tplus)
...
def time_to_fail_numerical(u,tstart,tmax,sigma,sigma_th,sigma_r,p,s,alpha,debug_level=-1):
    ...
    integr = scipy.integrate.solve_ivp(calc_rates_du_t,(tstart,tmax),y0 = np.array([0.0]),\
                                      ...
                                      max_step = -1e-1/p.v,atol=1e-14,rtol = 1e-12)
    if(debug_level > 1):
        print(integr)
...
def load_to_fail_numerical(mf,rng,tmax=1e99,debug_level = -1):
    ...
    if(False):
        if(len(ind_array) >0):
            for i in range(mf.N):
                if(sorted_list[i][0] == ind_array[0]):
                    break 
            print('failing site was at index  ', i)
