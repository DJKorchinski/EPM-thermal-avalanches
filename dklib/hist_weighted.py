...
class hist:
    def __init__(self,bins):
        self.bins = bins 
        self.counts = np.zeros(np.shape(bins)[0]-1,dtype=np.float64) #do we want an integer type here? 
        self.avg = avg()
    ...
    def addDat(self,dat,weight = 1.0,destructive=False):
        tempcounts,_ = np.histogram(dat,self.bins,weights=weight)
