...
class avalanche:
    ...
    STRESS_DISSIPATED_INDEX = 6
    def __init__(self,mf=None):
        self.values = np.zeros(shape = (2,avalanche.NUM_VALUES))
        if(not (mf is None)):
            self.update(mf)
        
    def update(self,mf,index = 0,st_count = -1,total_stress_dissipated=None):
        ...
        if(not (total_stress_dissipated is None)):
            self.values[index,avalanche.STRESS_DISSIPATED_INDEX] = total_stress_dissipated
