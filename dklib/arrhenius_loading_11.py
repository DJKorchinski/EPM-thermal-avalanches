...
import numba 
from . import mf_epm_03 as mf_epm
...
def load_to_fail_T0_thermal(mf : mf_epm.MFEPM,debug_level : int = -1):
    mf.compute_x()
    sites = [np.argmin(mf.x)]
    time = 0.
    return np.array(sites),time

def load_to_fail_T0_mechanical(mf : mf_epm.MFEPM, debug_level : int = -1):
    Nrange = np.arange(mf.N)
    #zeroeth order: are there sites that are currently in an invalid state, without us needing to do anything?
    #ideally, this should only happen at the start of the simulation when we apply a load!
    filt = (np.abs(mf.sigma) > mf.sigma_th) & (mf.tsolid == 0)
    inds = Nrange[filt]
    if(inds.size > 0):
        #then just fail immediately:
        print('took shortcut with: ',inds.size,' failing')
        return inds,0.0


    #first, checking the positive failing sites: 
    filt = ((mf.sigma+mf.sigma_res) > mf.sigma_th ) & (mf.sigma_res > 0) & (mf.sigma < mf.sigma_th)
    inds = Nrange[filt]
    first_t_pos,first_ind_pos = np.inf,-1 
    if(inds.size>0):
        tfail_positive = -mf.params.tau * np.log( (mf.sigma[filt]+mf.sigma_res[filt] - mf.sigma_th[filt])/mf.sigma_res[filt] )
        subfilt = (tfail_positive >= mf.tsolid[filt]) #checking only amongst those sites with a threshold crossing after they become solid.
        inds = inds[subfilt]
        if(inds.size>0):
            first_ind_in_subfilter = np.argmin(tfail_positive[subfilt])
            first_ind_pos = inds[first_ind_in_subfilter]
            first_t_pos = (tfail_positive[subfilt])[first_ind_in_subfilter]
    pos_tup = first_t_pos,first_ind_pos

    #next, checking the negative failing sites: 
    filt = ((-(mf.sigma+mf.sigma_res)) > mf.sigma_th) & (mf.sigma_res < 0) & (-mf.sigma < mf.sigma_th)
    inds = Nrange[filt]
    first_t_neg,first_ind_neg = np.inf,-1
    if(inds.size>0):
        tfail_negative = -mf.params.tau * np.log( (mf.sigma[filt]+mf.sigma_res[filt] + mf.sigma_th[filt])/mf.sigma_res[filt] )
        subfilt = tfail_negative >= mf.tsolid[filt]
        inds = inds[subfilt]
        if(inds.size>0):
            first_ind_in_subfilter = np.argmin(tfail_negative[subfilt])
            first_ind_neg = inds[first_ind_in_subfilter]
            first_t_neg = (tfail_negative[subfilt])[first_ind_in_subfilter]

    neg_tup = first_t_neg,first_ind_neg

    #finally, checking to see if any of the currently liquid sites will fail immediately upon resolidification:
    filt = ~mf.solid_site_inds() #amongst liquid sites:
    sigma_on_resolidification = mf.sigma[filt] + mf.sigma_res[filt]*(1 - np.exp(-mf.tsolid[filt] / mf.params.tau))
    filt_fails = np.abs(sigma_on_resolidification) >= mf.sigma_th[filt]  
    failing_inds = (Nrange[filt])[filt_fails]
    refail_time,refail_ind = np.inf,-1
    if(failing_inds.size>0):
        refail_ind = failing_inds[np.argmin(mf.tsolid[failing_inds])]
        refail_time = mf.tsolid[refail_ind]
    refail_tup = refail_time,refail_ind

    #now, checking through the three failure modes to see if any of them result in valid failures!
    sites = []
    least_failing_time = np.inf
    tups = [pos_tup,neg_tup,refail_tup]
    for tup in tups:
        t,ind = tup 
        if(t<least_failing_time):
            sites=[ind]
            least_failing_time=t

    #now, we advance time:
    if(t<np.inf):
        #a site actually failed mechanically!
        mf.advance_time(t)
        stress_decay = np.exp(-t / mf.params.tau) 
        mf.sigma +=  mf.sigma_res*(1-stress_decay)
        mf.sigma_res *= stress_decay     
    else:
        #nothing failed, so put system into equilibration (solidify all sites, transfer all residual stress to the stress variabels)
        mf.advance_time( mf.fluidization_time()*1.1 ) #force everything to be solid again
        mf.sigma+=mf.sigma_res 
        mf.sigma_res = 0 
    mf.compute_x()

    #check to see if we made an error and any other sites failed! Likely unecessary, but let's just be careful.
...
def calc_rates_inst_single(sigma,sigma_th,lmbda,beta,s,alpha):
    return lmbda*np.exp(-beta*np.maximum(sigma_th- s * sigma,0)**alpha)
...
def time_to_fail_numerical(u,tstart,tmax,sigma,sigma_th,sigma_r,p,s,alpha,debug_level=-1):
    ...
    if(np.size(integr.t_events) > 0):
        ...
        if(integr.t_events[1].size > 0):
            tfail = min(integr.t_events[1][0],tfail)
...
def load_to_fail_numerical(mf,rng,tmax=1e99,debug_level = -1):
    ...
    #doing a sanity check: what if we mess up and still have sites with negative 'x' that should be solidified by now?  I guess we make them fail now.. Better late than never? 
    solid_sites_that_are_failing = np.where( (mf.x <= 0) &  (mf.solid_site_inds()) )[0]
    for site in solid_sites_that_are_failing:
        if(not (site in ind_array)):
            ind_array.append(site)

            compute_in_loop_str=False
            if(np.size(sorted_list)>0):
                compute_in_loop_str = site in np.array(sorted_list)[:,0]
            
            if(not(tmin == Tplus[site])and debug_level > 0):
                print('We missed a site that should have failed: ',site,'tmin: %.2g'%tmin,'x: %.2g'%mf.x[site],\
                    'x @ tstart: %.2g'%-(np.abs(sigma_at_tstart[site])-mf.sigma_th[site]),'tstart: %.2g'%tstarts[site],'Tplus: %.2g'%Tplus[site], sites_immediately_failing[site],\
                        'should compute', sites_to_compute[site],'computed in loop: ',compute_in_loop_str)
            #so, why are we missing sites that should have failed? 
    
    if(np.size(ind_array)>1 and debug_level > 0):
        print('new ind array: ',ind_array)
    # if(np.size(ind_array)>20):
    #     import pdb;pdb.set_trace()
