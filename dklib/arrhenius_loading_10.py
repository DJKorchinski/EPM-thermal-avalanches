...
def load_to_fail_numerical(mf,rng,tmax=1e99,debug_level = -1):
    ...
    solid_sites_that_are_failing = np.where( (mf.x <= 0) &  (mf.tsolid>=0) )
    for site in solid_sites_that_are_failing:
        if(not (site in ind_array)):
            ind_array.append(site)
            print('We missed a site that should have failed: ',site,mf.x[site],mf.tsolid[site],'new ind array',ind_array,)
